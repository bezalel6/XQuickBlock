...
class StateManager<T extends object> {
  ...
  subscribeToAnyUpdates(callback: Callback<T>) {
    ...
    return () => {
      ...
      this.anyChangeCallbacks = this.anyChangeCallbacks.filter((_, i) => i !== l - 1);
    };
  }
  /**
...
class SettingsManager<T extends End> extends StateManager<ExtensionSettings> {
  ...
  private messageHandlers: Map<ExtensionMessage['type'], MessageHandler> = new Map();

  private constructor(
    initialState: ExtensionSettings,
    readonly end: End
  ) {
    super(initialState);
  ...
  async update(newState: Partial<ExtensionSettings>, shouldUpdateOtehrs = true) {
      ...
      (message: InternalExtensionMessage, sender, sendResponse) => {
        ...
        const handleMessage = async (notMyCircus: boolean) => {
          try {
            ...
            if (handler) {
              try {
                log('SettingsManager', `Executing handler for message type: ${message.type}`);
                return handler(message, sender, sendResponse);
              } catch (error) {
                log('SettingsManager', `Error handling message ${message.type}`, error);
                return { success: false, error: error.message };
              }
            } else {
              log('SettingsManager', `No handler found for message type: ${message.type}`);
              return {
                success: false,
            ...
            log('SettingsManager', `Unexpected error in message handling`, error);
            return { success: false, error: error.message };
          }
        };
        ...
        handleMessage(!isMyCircus)
          .then(response => {
            ...
            log('SettingsManager', `Sending response for ${message.type}`, response);
   ...
   */
  registerMessageHandler(type: ExtensionMessage['type'], handler: MessageHandler) {
...
export type SettingsManger = Awaited<ReturnType<typeof SettingsManager.getInstance>>;
